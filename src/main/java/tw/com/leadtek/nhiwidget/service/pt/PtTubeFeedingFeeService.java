...
import tw.com.leadtek.nhiwidget.sql.PtTubeFeedingFeeDao;
import tw.com.leadtek.tools.Utility;

@Service
public class PtTubeFeedingFeeService extends BasicPaymentTerms {

    @Autowired
    private PaymentTermsDao paymentTermsDao;
    @Autowired
    private PtTubeFeedingFeeDao ptTubeFeedingFeeDao;
    
    private String Category = "管灌飲食費"; 
    
    public java.util.Map<String, Object> findTubeFeedingFee(long ptId) {
        java.util.Map<String, Object> retMap;
        if (ptId > 0) {
            java.util.Map<String, Object> master = paymentTermsDao.findPaymentTerms(ptId, this.Category);
            if (!master.isEmpty()) {
                java.util.Map<String, Object> detail = ptTubeFeedingFeeDao.findOne(ptId);
    ...
    public long addTubeFeedingFee(PtTubeFeedingFeePl params) {
    ...
    public int updateTubeFeedingFee(long ptId, PtTubeFeedingFeePl params) {
        int ret = 0;
        java.util.Date start_date = Utility.detectDate(String.valueOf(params.getStart_date()));
        java.util.Date end_data = Utility.detectDate(String.valueOf(params.getEnd_date()));
        if (ptId>0) {
