...
public interface DEDUCTED_NOTEDao extends JpaRepository<DEDUCTED_NOTE, Long>, JpaSpecificationExecutor<DEDUCTED_NOTE> {
	...
	@Query(value = "SELECT * FROM  "
			...
			+ "(SELECT COALESCE(SUM(CAST(VAL AS INT)),0) AS EXTRACTCASE FROM PARAMETERS WHERE NOTE = '核刪抽件數'  AND TO_CHAR(START_DATE,'YYYYMM') <= ?1 AND ?2 <= TO_CHAR(START_DATE,'YYYYMM'))o"
			+ "", nativeQuery = true)
