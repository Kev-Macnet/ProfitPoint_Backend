...
public interface OP_DDao extends JpaRepository<OP_D, Long>, JpaSpecificationExecutor<OP_D> {
  ...
  @Query(value = "SELECT * FROM " + 
      ...
      + "(SELECT COUNT(1) AS DRG_QUANTITY FROM MR WHERE DRG_SECTION IS NOT NULL AND DATA_FORMAT='20' AND APPL_YM=?1) l,"
      + "(SELECT SUM(MR.T_DOT) AS DRG_APPLDOT, SUM(IP_D.MED_DOT) AS DRG_ACTUAL_POINT FROM MR, IP_D WHERE "
      + "MR.DRG_SECTION IS NOT NULL AND MR.DATA_FORMAT='20' AND MR.APPL_YM=?1 AND MR.ID = IP_D.MR_ID) m,"
  ...
  @Query(value = "SELECT * FROM " + 
      ...
      + "(SELECT COUNT(1) AS DRG_QUANTITY FROM MR WHERE DRG_SECTION IS NOT NULL AND DATA_FORMAT='20' AND MR_END_DATE LIKE CONCAT(?1,'%')) l,"
      + "(SELECT SUM(MR.T_DOT) AS DRG_APPLDOT, SUM(IP_D.MED_DOT) AS DRG_ACTUAL_POINT FROM MR, IP_D WHERE "
      + "MR.DRG_SECTION IS NOT NULL AND MR.DATA_FORMAT='20'  AND MR.MR_END_DATE LIKE CONCAT(?1,'%') AND MR.ID = IP_D.MR_ID) m,"
